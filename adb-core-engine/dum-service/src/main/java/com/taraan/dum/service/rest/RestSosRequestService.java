...
public class RestSosRequestService {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response create(@Context HttpHeaders headers, SosRequestDto sosRequestDto) {
        List<String> authentication = headers.getRequestHeader("authentication");
        Long userId = TokenUtils.getId(authentication.get(0));
        final SosRequestDto sosRequest = sosRequestAssembler.createSosRequest(sosRequestDto, userId);
        return Response.ok().entity(new SosRequestResponse(sosRequest.getId())).build();
    }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response gets(@Context HttpHeaders headers, @PathParam("page") Long page) {
        List<String> authentication = headers.getRequestHeader("authentication");
        Long userId = TokenUtils.getId(authentication.get(0));
        return Response.ok().entity(sosRequestAssembler.getSosRequests(page)).build();
    }
